...
const int INTENSITY_PER_LIGHT = 4;
...
GLfloat getTile(int tileX, int tileY)
{
	...
	if (tileX < 0) return 1;
	if (tileY < 0) return 1;
	if (tileX >= TILES_WIDE) return 1;
	if (tileY >= TILES_HIGH) return 1;
...
void setPixel(int x, int y, GLfloat r, GLfloat g, GLfloat b)
{
	...
	frameBuffer[pos + RED_OFS] = r;
	frameBuffer[pos + GREEN_OFS] = g;
	frameBuffer[pos + BLUE_OFS] = b;
}

// ----------------------------------------------------------
bool lineOfSight(int x0, int y0, int x1, int y1)
{
#ifdef OPT_CAST_FORWARD
	if (y0 > y1)
	{
		int xtmp = x0;
		int ytmp = y0;
...
void spawnLights(void)
{
	...
	// *** sort the lights ***
#ifdef OPT_SORT_LIGHTS  
	int i, j;
	int iMin;

	for (j = 0; j < TOTAL_LIGHTS - 1; j++) {
		iMin = j;
		/* test against elements after j to find the smallest */
		for (i = j + 1; i < TOTAL_LIGHTS; i++) {
			/* if this element is less, then it is the new minimum */
			if ((lights[i].tileY * TILES_WIDE + lights[i].tileX) < (lights[iMin].tileY * TILES_WIDE + lights[iMin].tileX)) {
		...
		if (iMin != j) {
			RadLight tmp = lights[j];
			lights[j] = lights[iMin];
			lights[iMin] = tmp;
		}
	...
	//      printf("Lights[%d] = %d %d\n", i, lights[i].tileX, lights[i].tileY);
		...
		for (int j = j_start; j<j_end; j++)
		{
			...
			if (getTile(i, j) != 1)
			{
				for (int n = 0; n<TOTAL_LIGHTS; n++)
					if (lineOfSight(i, j, lights[n].tileX, lights[n].tileY))
						intensity += INTENSITY_PER_LIGHT;
...
void threads_render(){
	...
	threads.push_back(std::thread(render_scene_thread, 0, (TILES_WIDE / 4), 0, TILES_HIGH / 4));
	threads.push_back(std::thread(render_scene_thread, 0, (TILES_WIDE / 4), TILES_WIDE/4, TILES_HIGH / 2));

	threads.push_back(std::thread(render_scene_thread, TILES_WIDE/4, (TILES_WIDE / 2), 0, TILES_HIGH / 4));
	threads.push_back(std::thread(render_scene_thread, TILES_WIDE / 4, (TILES_WIDE / 2), TILES_WIDE/4, TILES_HIGH / 2));

	threads.push_back(std::thread(render_scene_thread, (TILES_WIDE / 2), (3/4)*TILES_WIDE, 0, (TILES_HIGH / 4)));
	threads.push_back(std::thread(render_scene_thread, (TILES_WIDE / 2), (3 / 4)*TILES_WIDE, TILES_HIGH/4, (TILES_HIGH / 2)));

	threads.push_back(std::thread(render_scene_thread, (3/4)*TILES_WIDE, TILES_WIDE, 0, (TILES_HIGH / 4)));
	threads.push_back(std::thread(render_scene_thread, (3 / 4)*TILES_WIDE, TILES_WIDE, TILES_HIGH/4, (TILES_HIGH / 2)));


	threads.push_back(std::thread(render_scene_thread, TILES_WIDE/2, (3/4) *TILES_WIDE, TILES_HIGH/2, (3/4)*TILES_HIGH));
	threads.push_back(std::thread(render_scene_thread, TILES_WIDE / 2, (3 / 4) *TILES_WIDE, (3/4)*TILES_HIGH , TILES_HIGH));

	threads.push_back(std::thread(render_scene_thread, (3/4) * TILES_WIDE, TILES_WIDE, TILES_HIGH / 2, (3/4)*TILES_HIGH));
	threads.push_back(std::thread(render_scene_thread, (3 / 4) * TILES_WIDE, TILES_WIDE, (3/4)* TILES_HIGH , TILES_HIGH));

	threads.push_back(std::thread(render_scene_thread, 0, (TILES_WIDE / 4), TILES_HIGH/2, (3/4)* TILES_HIGH));
	threads.push_back(std::thread(render_scene_thread, 0, (TILES_WIDE / 4), (3/4) *TILES_HIGH, TILES_HIGH));

	threads.push_back(std::thread(render_scene_thread, TILES_WIDE/4, (TILES_WIDE / 2), TILES_HIGH / 2, (3/4) * TILES_HIGH));
	threads.push_back(std::thread(render_scene_thread, TILES_WIDE / 4, (TILES_WIDE / 2), (3/4) * TILES_HIGH, TILES_HIGH));
...
void cast_light_thread(int i_start, int i_end, int j_start, int j_end){
	// cast light to each pixel
	for (int i = i_start; i < i_end; i++)
	{
		for (int j = j_start; j < j_end; j++)
...
void cast_light(){
	...
	threads.push_back(std::thread(cast_light_thread, 0, TILES_WIDE/2, 0, TILES_HIGH/2));
	threads.push_back(std::thread(cast_light_thread, 0, TILES_WIDE / 2, TILES_HIGH/2, TILES_HIGH));

	threads.push_back(std::thread(cast_light_thread, TILES_WIDE/2, TILES_WIDE, 0, TILES_HIGH/2));
	threads.push_back(std::thread(cast_light_thread, TILES_WIDE / 2, TILES_WIDE, TILES_HIGH/2, TILES_HIGH));
